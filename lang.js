/* ══════════════════════════════════════════════════════════════
   12. ПЕРЕКЛЮЧЕНИЕ ЯЗЫКОВ (только интерфейс, товары не трогаем)
   ══════════════════════════════════════════════════════════════ */
let currentLang = 'ru';
...
window.switchLang = function(lang) {
  ...
  // Перевод кнопки корзины
  const cartLabel = document.querySelector('.cart-label');
  if (cartLabel) cartLabel.innerHTML = t[lang].cart;
  
  // Перевод кнопок hero
  const btnPrimary = document.querySelector('.btn-primary');
  if (btnPrimary) btnPrimary.innerHTML = t[lang].btn_catalog + ' <svg width="16" height="16" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>';
  
  const btnSecondary = document.querySelector('.btn-secondary');
  if (btnSecondary) btnSecondary.innerHTML = t[lang].btn_contact;
  
  // Перевод фильтров кондиционеров
